package huihuang.proxy.ocpx.middle.impl;

import huihuang.proxy.ocpx.ads.huihui.HuihuiParamField;
import huihuang.proxy.ocpx.ads.huihui.HuihuiPath;
import huihuang.proxy.ocpx.bussiness.dao.ads.IHuihuiMomoAdsDao;
import huihuang.proxy.ocpx.bussiness.dao.ads.IHuihuiXianyuAdsDao;
import huihuang.proxy.ocpx.common.Constants;
import huihuang.proxy.ocpx.marketinterface.IMarkDao;
import huihuang.proxy.ocpx.middle.baseadsreport.huihuiyoudao.IQiyiHuihuiReportFactory;
import huihuang.proxy.ocpx.middle.baseadsreport.huihuiyoudao.OppoHuihuiReportFactory;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

import java.util.Map;

@Component("oppoHuihuiMomoChannelAds")
public class OppoHuihuiMomoChannelAds extends OppoHuihuiReportFactory {

    @Autowired
    private IHuihuiMomoAdsDao huihuiMomoAdsDao;

    String channelAdsKey = Constants.ChannelAdsKey.OPPO_HUIHUI_MOMO;


    @Override
    protected String channelAdsKey() {
        return channelAdsKey;
    }

    @Override
    protected String serverPathKey() {
        return Constants.ServerPath.OPPO_HUIHUI_MOMO;
    }

    @Override
    protected IMarkDao adsDao() {
        return huihuiMomoAdsDao;
    }


    @Override
    protected String initAdsUrl() {
        return HuihuiPath.BASIC_URI_2;
    }

    @Override
    protected Object channelParamToAdsParam(Map<String, String[]> parameterMap) {
        HuihuiParamField huihuiParamField = (HuihuiParamField) super.channelParamToAdsParam(parameterMap);
        logger.info("clickReport {} 媒体侧请求的监测链接中的参数，转化成广告侧的参数对象 channelParamToAdsParam:{}", channelAdsKey, huihuiParamField);
        huihuiParamField.setRedirect("false");
//        huihuiParamField.setConv_ext("Ego0ODY1NDM5NzIyGAEgAiqLA2h0dHBzOi8vbWt0LWFkdmVydGlzZW1lbnQudHVodS5jbi9leHQvYWR2ZXJ0aXNlbWVudC93YW5neWkvY2xpY2s__YcDcIKLlww");
        return huihuiParamField;
    }

}
