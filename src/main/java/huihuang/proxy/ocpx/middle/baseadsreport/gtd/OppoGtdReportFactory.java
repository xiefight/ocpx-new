...
public abstract class OppoGtdReportFactory extends GtdReportFactory {
    ...
    public String findMonitorAddress() {
        ...
        //1.遍历广告主查找渠道对应的宏参数
        Set<GtdParamEnum> gtdParamEnums = GtdParamEnum.gtdOppoMap.keySet();
        for (GtdParamEnum gtd : gtdParamEnums) {
            OppoParamEnum oppo = GtdParamEnum.gtdOppoMap.get(gtd);
    ...
    protected Object channelParamToAdsParam(Map<String, String[]> parameterMap) {
        ...
        Set<Map.Entry<GtdParamEnum, OppoParamEnum>> hbSet = GtdParamEnum.gtdOppoMap.entrySet();
        hbSet.stream().filter(hb -> Objects.nonNull(hb.getValue())).forEach(hb -> {
            GtdParamEnum gtdEnum = hb.getKey();
            OppoParamEnum oppoEnum = hb.getValue();
            String gtdField = gtdEnum.getName();
            String oppoParam = oppoEnum.getParam();
