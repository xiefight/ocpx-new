...
public abstract class HuihuangHuihuangReportFactory extends HuihuangMingtianReportFactory {
    ...
    public String findMonitorAddress() {
        ...
        //1.遍历广告主查找渠道对应的宏参数
        Set<HuihuangmingtianParamEnum> huihuangmingtianParamEnums = HuihuangmingtianParamEnum.huihuangmingtianHuihuangMap.keySet();
        for (HuihuangmingtianParamEnum huihuangmingtian : huihuangmingtianParamEnums) {
            HuihuangChannelParamEnum hhmt = HuihuangmingtianParamEnum.huihuangmingtianHuihuangMap.get(huihuangmingtian);
            if (Objects.isNull(hhmt) || StrUtil.isEmpty(hhmt.getMacro())) {
                continue;
            }
            macro.append(hhmt.getParam()).append("=").append(hhmt.getMacro()).append("&");
        }
    ...
    protected Object channelParamToAdsParam(Map<String, String[]> parameterMap) {
        ...
        Set<Map.Entry<HuihuangmingtianParamEnum, HuihuangChannelParamEnum>> hbSet = HuihuangmingtianParamEnum.huihuangmingtianHuihuangMap.entrySet();
        hbSet.stream().filter(hb -> Objects.nonNull(hb.getValue())).forEach(hb -> {
            HuihuangmingtianParamEnum huihuangmingtian = hb.getKey();
            HuihuangChannelParamEnum hhmtChannel = hb.getValue();
            String huihuangmingtianField = huihuangmingtian.getName();
            String hhmtParam = hhmtChannel.getParam();
            String[] value = parameterMap.get(hhmtParam);
