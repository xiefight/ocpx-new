...
@RestController
@RequestMapping(Constants.ServerPath.OPPO_HUIHUANG_ELEME)
public class OppoHuihuangElemeController {

    @Autowired
    @Qualifier("oppohhelemeService")
    private IChannelAdsService oppohhelemeService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return oppohhelemeService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public Response clickReport(HttpServletRequest request) {
        try {
            ...
            return oppohhelemeService.clickReport(parameterMap);
        } catch (Exception e) {
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
    }
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return oppohhelemeService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
