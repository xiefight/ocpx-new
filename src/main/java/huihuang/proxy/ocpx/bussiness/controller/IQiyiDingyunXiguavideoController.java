...
@RestController
@RequestMapping(Constants.ServerPath.IQIYI_DINGYUN_XIGUAVIDEO)
public class IQiyiDingyunXiguavideoController {

    @Autowired
    @Qualifier("idyxgService")
    private IChannelAdsService idyxgService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return idyxgService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public XiaomiResponse clickReport(HttpServletRequest request) {
        ...
        try {
            ...
            response = idyxgService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return idyxgService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
