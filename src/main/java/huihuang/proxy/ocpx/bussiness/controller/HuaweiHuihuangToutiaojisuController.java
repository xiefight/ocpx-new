...
@RestController
@RequestMapping(Constants.ServerPath.HUAWEI_HUIHUANG_TOUTIAOJISU)
public class HuaweiHuihuangToutiaojisuController {

    @Autowired
    @Qualifier("hhhttjsService")
    private IChannelAdsService hhhttjsService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return hhhttjsService.monitorAddress(params);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Response clickReport(HttpServletRequest request) {
        try {
            ...
            return hhhttjsService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return hhhttjsService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
