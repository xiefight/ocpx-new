...
@RestController
@RequestMapping(Constants.ServerPath.XIAOMI_QUANNENG_DOUYINJISU)
public class XiaomiQuannengDouyinjisuController {

    @Autowired
    @Qualifier("xqdyjsService")
    private IChannelAdsService xqdyjsService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return xqdyjsService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public XiaomiResponse clickReport(HttpServletRequest request) {
        ...
        try {
            ...
            response = xqdyjsService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return xqdyjsService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
