...
@RestController
@RequestMapping(Constants.ServerPath.XIAOMI_QIDU)
public class XiaomiQiduController {

    @Autowired
    @Qualifier("xqiduService")
    private IChannelAdsService xqiduService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return xqiduService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public XiaomiResponse clickReport(HttpServletRequest request) {
        ...
        try {
            ...
            response = xqiduService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return xqiduService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
