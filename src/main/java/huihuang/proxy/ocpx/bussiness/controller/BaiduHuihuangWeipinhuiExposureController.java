 ...
 */
@RestController
@RequestMapping(Constants.ServerPath.BAIDU_HUIHUANG_WEIPINHUI_EXPOSURE)
public class BaiduHuihuangWeipinhuiExposureController {

    @Autowired
    @Qualifier("bhhweipinhuiExposureService")
    private IChannelAdsService bhhweipinhuiExposureService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return bhhweipinhuiExposureService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public Response clickReport(HttpServletRequest request) {
        try {
            ...
            return bhhweipinhuiExposureService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return bhhweipinhuiExposureService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
