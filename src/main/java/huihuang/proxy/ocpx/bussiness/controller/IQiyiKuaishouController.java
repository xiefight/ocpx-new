 ...
 */
@RestController
@RequestMapping(Constants.ServerPath.IQIYI_KUAISHOU)
public class IQiyiKuaishouController {

    @Autowired
    @Qualifier("ikService")
    private IChannelAdsService ikService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return ikService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public XiaomiResponse clickReport(HttpServletRequest request) {
        ...
        try {
            ...
            response = ikService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return ikService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
