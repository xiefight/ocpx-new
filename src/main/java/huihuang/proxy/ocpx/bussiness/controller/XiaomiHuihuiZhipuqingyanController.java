...
@RestController
@RequestMapping(Constants.ServerPath.XIAOMI_HUIHUI_ZHIPUQINGYAN)
public class XiaomiHuihuiZhipuqingyanController {

    @Autowired
    @Qualifier("xmhhzpqyService")
    private IChannelAdsService xmhhzpqyService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return xmhhzpqyService.monitorAddress(params);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Response clickReport(HttpServletRequest request) {
        try {
            ...
            return xmhhzpqyService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return xmhhzpqyService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
