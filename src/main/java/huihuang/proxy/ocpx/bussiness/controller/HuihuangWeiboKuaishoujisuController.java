...
@RestController
@RequestMapping(Constants.ServerPath.HUIHUANG_WEIBO_KUAISHOUJISU)
public class HuihuangWeiboKuaishoujisuController {

    @Autowired
    @Qualifier("hhwbksjsService")
    private IChannelAdsService hhwbksjsService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return hhwbksjsService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public Response clickReport(HttpServletRequest request) {
        try {
            ...
            return hhwbksjsService.clickReport(parameterMap);
        } catch (Exception e) {
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
    }
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return hhwbksjsService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
