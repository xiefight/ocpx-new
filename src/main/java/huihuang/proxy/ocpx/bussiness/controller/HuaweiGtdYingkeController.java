...
@RestController
@RequestMapping(Constants.ServerPath.HUAWEI_GTD_YINGKE)
public class HuaweiGtdYingkeController {
    protected Logger logger = LoggerFactory.getLogger(HuaweiGtdYingkeController.class);

    @Autowired
    @Qualifier("hwgtdykService")
    private IChannelAdsService hwgtdykService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        ...
        try {
            ...
            return hwgtdykService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public Response clickReport(HttpServletRequest request) {
        try {
            ...
            return hwgtdykService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return hwgtdykService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
