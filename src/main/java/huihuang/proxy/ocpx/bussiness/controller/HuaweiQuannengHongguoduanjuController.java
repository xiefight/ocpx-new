...
@RestController
@RequestMapping(Constants.ServerPath.HUAWEI_QUANNENG_HONGGUODUANJU)
public class HuaweiQuannengHongguoduanjuController {

    @Autowired
    @Qualifier("hqhgdjService")
    private IChannelAdsService hqhgdjService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return hqhgdjService.monitorAddress(params);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Response clickReport(HttpServletRequest request) {
        try {
            ...
            return hqhgdjService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return hqhgdjService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
