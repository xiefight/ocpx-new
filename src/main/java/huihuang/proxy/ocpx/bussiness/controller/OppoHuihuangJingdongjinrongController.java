...
@RestController
@RequestMapping(Constants.ServerPath.OPPO_HUIHUANG_JINGDONGJINRONG)
public class OppoHuihuangJingdongjinrongController {

    @Autowired
    @Qualifier("oppohhjdjrService")
    private IChannelAdsService oppohhjdjrService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return oppohhjdjrService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public Response clickReport(HttpServletRequest request) {
        try {
            ...
            return oppohhjdjrService.clickReport(parameterMap);
        } catch (Exception e) {
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
    }
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return oppohhjdjrService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
