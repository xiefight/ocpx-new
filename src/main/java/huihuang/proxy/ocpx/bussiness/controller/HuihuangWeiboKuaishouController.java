...
@RestController
@RequestMapping(Constants.ServerPath.HUIHUANG_WEIBO_KUAISHOU)
public class HuihuangWeiboKuaishouController {

    @Autowired
    @Qualifier("hhwbksService")
    private IChannelAdsService hhwbksService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return hhwbksService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public Response clickReport(HttpServletRequest request) {
        try {
            ...
            return hhwbksService.clickReport(parameterMap);
        } catch (Exception e) {
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
    }
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return hhwbksService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
