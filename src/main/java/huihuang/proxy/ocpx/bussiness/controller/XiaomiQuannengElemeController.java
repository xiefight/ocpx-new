...
@RestController
@RequestMapping(Constants.ServerPath.XIAOMI_QUANNENG_ELEME)
public class XiaomiQuannengElemeController {

    @Autowired
    @Qualifier("xqelemeService")
    private IChannelAdsService xqelemeService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return xqelemeService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public XiaomiResponse clickReport(HttpServletRequest request) {
        ...
        try {
            ...
            response = xqelemeService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return xqelemeService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
