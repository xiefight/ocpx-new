...
@RestController
@RequestMapping(Constants.ServerPath.BAIDU_HUIHUANG_DOUYINHUOSHAN)
public class BaiduHuihuangDouyinhuoshanController {

    @Autowired
    @Qualifier("bhhdyhsService")
    private IChannelAdsService bhhdyhsService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return bhhdyhsService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public Response clickReport(HttpServletRequest request) {
        try {
            ...
            return bhhdyhsService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return bhhdyhsService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
