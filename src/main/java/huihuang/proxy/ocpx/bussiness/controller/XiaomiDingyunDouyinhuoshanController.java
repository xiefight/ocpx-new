...
@RestController
@RequestMapping(Constants.ServerPath.XIAOMI_DINGYUN_DOUYINHUOSHAN)
public class XiaomiDingyunDouyinhuoshanController {

    @Autowired
    @Qualifier("xdydyhsService")
    private IChannelAdsService xdydyhsService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return xdydyhsService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public XiaomiResponse clickReport(HttpServletRequest request) {
        ...
        try {
            ...
            response = xdydyhsService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return xdydyhsService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
