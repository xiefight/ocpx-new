...
@RestController
@RequestMapping(Constants.ServerPath.BAIDU_QUANNENG_DOUYIN_JISU)
public class BaiduQuannengDouyinjisuController {

    @Autowired
    @Qualifier("bqdyjsService")
    private IChannelAdsService bqdyjsService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return bqdyjsService.monitorAddress(params);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Response clickReport(HttpServletRequest request) {
        try {
            ...
            return bqdyjsService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return bqdyjsService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
