...
@RestController
@RequestMapping(Constants.ServerPath.XIAOMI_BUPET_BILI)
public class XiaomiBupetBiliController {

    @Autowired
    @Qualifier("xmbupetbiliService")
    private IChannelAdsService xmbupetbiliService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return xmbupetbiliService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public Response clickReport(HttpServletRequest request) {
        ...
        try {
            ...
            response = xmbupetbiliService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return xmbupetbiliService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
