 ...
 */
@RestController
@RequestMapping(Constants.ServerPath.BAIDU_HUIHUI_HEMADUANJU)
public class BaiduHuihuiHemaduanjuController {

    @Autowired
    @Qualifier("bdhhhemaduanjuService")
    private IChannelAdsService bdhhhemaduanjuService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return bdhhhemaduanjuService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public Response clickReport(HttpServletRequest request) {
        try {
            ...
            return bdhhhemaduanjuService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return bdhhhemaduanjuService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
