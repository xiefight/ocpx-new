...
/**
 * @Description: wifi-xigua
 * @Author: xietao
 * @Date: 2023-05-09 20:24
 **/
@RestController
@RequestMapping(Constants.ServerPath.WIFI_XIGUA)
public class WifiXiguaController {

    @Autowired
    @Qualifier("wxService")
    private IChannelAdsService wxService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return wxService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public Response clickReport(HttpServletRequest request) {
        ...
        try {
            ...
            response = wxService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return wxService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
