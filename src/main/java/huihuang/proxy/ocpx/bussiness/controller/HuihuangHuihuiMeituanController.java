...
/**
 * 外放链接：将有道的美团外放，通过辉煌包装了一层
 * 在请求连接上使用huihui-meituan，在上报时使用huihuang-meituan
 */
@RestController
@RequestMapping(Constants.ServerPath.HUIHUANG_HUIHUI_MEITUAN)
public class HuihuangHuihuiMeituanController {

    @Autowired
    @Qualifier("hhhuihmtService")
    private IChannelAdsService hhhuihmtService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return hhhuihmtService.monitorAddress(params);
        } catch (Exception e) {
    ...
    public Response clickReport(HttpServletRequest request) {
        try {
            ...
            return hhhuihmtService.clickReport(parameterMap);
        } catch (Exception e) {
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
    }
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return hhhuihmtService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
