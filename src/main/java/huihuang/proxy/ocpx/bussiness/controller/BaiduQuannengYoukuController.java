...
@RestController
@RequestMapping(Constants.ServerPath.BAIDU_QUANNENG_YOUKU)
public class BaiduQuannengYoukuController {

    @Autowired
    @Qualifier("bdqnykService")
    private IChannelAdsService bdqnykService;
    ...
    public Response monitorAddress(HttpServletRequest request, @RequestBody String reqBody) {
        try {
            ...
            return bdqnykService.monitorAddress(params);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Response clickReport(HttpServletRequest request) {
        try {
            ...
            return bdqnykService.clickReport(parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
    ...
    public Response adsCallBack(HttpServletRequest request, @PathVariable Integer id) {
        try {
            Map<String, String[]> parameterMap = request.getParameterMap();
            return bdqnykService.adsCallBack(id, parameterMap);
        } catch (Exception e) {
            e.printStackTrace();
            return BasicResult.getFailResponse("请求异常", e.getMessage());
        }
